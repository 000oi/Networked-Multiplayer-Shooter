...
onready var sprite = $Survivor1Gun
onready var reload_timer = $Reload_timer
onready var shoot_point = $Shoot_point
onready var hit_timer = $Hit_timer
const speed:float=709.5
puppet var puppet_position = Vector2(0, 0) setget puppet_position_set
puppet var puppet_velocity = Vector2()
puppet var puppet_rotation = 0
puppet var puppet_hp = 250 setget puppet_hp_set
puppet var puppet_username = "" setget puppet_username_set
var velocity:=Vector2.ZERO
var can_shoot = true
var is_reloading = false
var player_bullet = load("res://Game project/Cenas/Player_bullet.tscn")
var username_text = load("res://Game project/Cenas/Username_text.tscn")
...
func _ready():
	...
	username_text_instance = Globals.instance_node_at_location(username_text, Persistent_nodes, global_position)
	username_text_instance.player_following = self
	update_shoot_mode(false)
	Globals.alive_players.append(self)
	yield(get_tree(), "idle_frame")
	if get_tree().has_network_peer():
		if is_network_master():
			Globals.player_master = self
...
func _process(delta):
	if get_tree().has_network_peer():
		if is_network_master() and visible:
			...
			move_and_slide(velocity * speed * delta * 16)
			...
			if not tween.is_active():
				move_and_slide(puppet_velocity * speed * delta * 16)
	if hp <= 0:
		if username_text_instance != null:
...
sync func instance_bullet(id):
	var player_bullet_instance = Globals.instance_node_at_location(player_bullet, Persistent_nodes, shoot_point.global_position)
...
sync func destroy() -> void:
	...
	Globals.alive_players.erase(self)
	if get_tree().has_network_peer():
		if is_network_master():
			Globals.player_master = null
...
sync func enable() -> void:
	...
	if get_tree().has_network_peer():
		if is_network_master():
			Globals.player_master = self
	if not Globals.alive_players.has(self):
		Globals.alive_players.append(self)
